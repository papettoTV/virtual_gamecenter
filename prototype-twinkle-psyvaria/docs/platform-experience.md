...
- 登録アカウントは、同じクレジット残高を別ブラウザ・PC・スマートフォンで共有できる
- 登録アカウントは、ランキングへ表示する名前をプロフィールで設定できる
- 未登録プレイヤーのランキング名は自由入力にせず、システムが匿名プレイヤーごとに自動発行した固定名を使用する

登録後の利用可能残高は、匿名プレイヤー時点の残高に登録特典5クレジットを加えた値とする。新しいウォレットへ残高をコピーするのではなく、匿名プレイヤーと登録アカウントを安全に関連付け、既存のクレジット台帳を継続して利用する。

### アカウント登録のメリット

1. 購入・無料クレジットを複数端末間で共有できる
2. ランキング登録名を自分で設定できる

ランキング登録名はゲーム結果送信時の自由入力値を信用せず、サーバー側でプレイヤー情報から決定する。登録済みの場合は設定したランキング名、未登録の場合は自動発行された固定名を使用する。

### 実装時期

クレジットの端末間共有とランキング名設定を継続的なメリットとして、アカウント登録機能の実装を進める。最初はGoogle IdPだけに対応し、匿名プレイヤーから登録アカウントへのデータ引継ぎを優先する。

## クレジット設計

### 購入フロー

1. 利用可能クレジットが0枚の状態でプレイ開始を選ぶ
2. 「クレジットを購入しますか？」で購入またはキャンセルを選ぶ
3. 購入時は1・3・5・10単位から選ぶ
4. 1単位は5クレジットで、日本円は100円、米ドルは1ドルとする
5. 10単位は購入特典として60クレジットを付与する
6. Stripe Checkoutでカード決済する
7. 署名検証済みの `checkout.session.completed` Webhookで支払い完了を確認する
8. D1の購入レコードと金額・通貨・Checkout Sessionを照合する
9. 購入クレジットを台帳へ一度だけ付与する
10. 完了画面を閉じると、購入前のクレジット投入画面へ戻る

ブラウザのCookieにはクレジット残高を保存しない。Cookieは匿名プレイヤーのセッショントークンだけを保持し、残高と購入履歴はD1へ保存する。匿名セッションは訪問時に期限を更新する1年間のローリング期限とする。別ブラウザ・PC・スマートフォンとの共有にはアカウント登録を必要とする。

決済完了画面ではアカウント登録の利点を案内する。アカウント登録機能が完成するまでは導線を「準備中」として表示する。
